...
#max number of tickets per run(3)
#configurable ticketstatusupdate
#
#V5
#correction to use utc time for calculation of ticket updates
#adding helpmatics logo to comment
#
...
wdsl = 'https://web.helpmatics.net/operationcenter/xella/portal.nsf/IncidentHandler?wsdl'
HM_username = 'wsxella'
HM_password = 'XXXXXXXXXXXX'
...
hmlogo='<img src="https://www.helpmatics.com/fileadmin/helpmatics/template/hm_logo.jpg" width="120" height="20"> \n'

#Icinga API data and API user see /etc/icinga2/conf.d/api-users.conf
icingaserver = 'https://icinga.xadone.com:5665'
icingauser = 'root'
icingapassword = 'XXXXXXXXXXXX'
#VARIABLES END

currenttime = datetime.datetime.utcnow()
scriptintervalseconds = scriptinterval.total_seconds()
hmlogolenght = len(hmlogo)
...
#     object_type = 'Host',
#     filters = 'host.name== "SDCAWG014"',
#     author = 'Helpmaticsscript',
#     comment = 'this is a test2')

# Icingaclient.actions.add_comment(
#     object_type = 'Service',
#     filters = 'host.name=="SDCAWG014" && service.name=="icmp"',
#     author = 'Helpmaticsscript',
    ...
    if comment['attrs']['text'].startswith(HM_incident_startswith):
        if test_hm_interface_status(comment['attrs']['text'][0:20]) is True :
            ...
            if invalid_hm_response_counter == 5:
                exit(3)
    elif comment['attrs']['text'].startswith(hmlogo + HM_incident_startswith):
        if test_hm_interface_status(comment['attrs']['text'][hmlogolenght + 0: hmlogolenght + 20]) is True :
            break
        else :
            invalid_hm_response_counter += 1
            #end script after 5 times failed to get correct HM response
...
for comment in Commentlist:
    ...
    if comment['attrs']['text'].startswith(HM_incident_startswith) or comment['attrs']['text'].startswith(hmlogo + HM_incident_startswith):
        ...
        if comment['attrs']['text'].startswith(HM_incident_startswith):
            ...
            HMticket = get_hm_ticket(comment['attrs']['text'][hmlogolenght + 0:hmlogolenght + 20])
        ...
        if HMticket['INC']['ID'] is None:
            ...
            newcomment = hmlogo + HMticket['INC']['ID'] + " : "  + convert_ticketstatus(HMticket['INC']['STATUS_NO'])  + \
            "\n\n<B><U>SOLUTION:</U></B>\n" + HMticket['INC']['SOLUTION'] + \
        ...
        # #for testing to only modify one comment
        # if icingaobject['attrs']['display_name'] != 'IcingaServer':
        #     print 'weiter'
        #     print HMticket['INC']['ID']
        #     continue
        # else:
        ...
        if int(HMticket['INC']['STATUS_NO']) <= 4 and hmstatusupdates:
            ...
            ticketcreationtime = datetime.datetime.utcfromtimestamp(time.mktime(datetime.datetime.strptime(HMticket['INC']['ID'][0:16] , "XOC-%y%m%d%H%M%S").timetuple()))
            #print icingaobject
            ...
            while currenttime > ticketcreationtime :
                ...
                if deltaseconds <= scriptintervalseconds and days_to_find_if_positive < 0 :
                    ...
                    NEWHMTicket_xml = HMclient.service.UPDATEINCIDENT(
                                    ID = HMTICKETID,
                                    USER_ID = 'SVC1Icinga_Sendmail@xella.com',
                                    CI_ID = '',
                                    BRIEFDESCRIPTION = '',
                                    REQUEST = '',
...
#print Icingaclient.objects.list('Host', filters='match("SDCAWG014", host.name)')
